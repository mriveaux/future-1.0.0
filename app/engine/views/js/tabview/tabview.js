        ...
        listeners: {
            click: function(nodo) {
                if (nodo.attributes.leaf === true && nodo.attributes.src.length > 10) {
                    ...
                    if (!Ext.getCmp('tabview').getItem('idTab' + String(nodo.attributes.id))) {
                        var newTab = {
                            id: 'idTab' + String(nodo.attributes.id),
                            title: getMenuName(nodo.attributes.label),
                            layout: 'fit',
                            closable: true,
                            plain: true,
                            items: [{
                                    ...
                                    border: false,
                                    layout: 'fit',
                                    html: '<iframe id="ifContent" style="width:100%; height: 100%; border:none;" src="' + nodo.attributes.src + '"></iframe>'
                                }]
                        };
                        Ext.getCmp('tabview').add(newTab);
                    }
                    Ext.getCmp('tabview').activate('idTab' + String(nodo.attributes.id));
                }
            }
        }
    });
...
function regUserRecomendation() {
    Ext.getCmp('tabview').setActiveTab(0);
