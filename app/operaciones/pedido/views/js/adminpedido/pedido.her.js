...
var btnRevisarHerramienta = new Ext.Button({
    id: 'btnRevisarHerramienta',
    text: '<i class="fa fa-search-plus bluedark-button"></i> Revisar',
    tooltip: 'Marcar para revisar',
    handler: function() {
        var record = Ext.getCmp('gppedidoHerramienta').getSelectionModel().getSelected();
        markToReviewHerramienta(Ext.getCmp('gppedidoHerramienta'), record, record);
    }
});
...
//    autoExpandColumn: 'idproducto',
    ...
    tbar: [btnAdicionarHerramienta, btnModificarHerramienta, btnEliminarHerramienta, btnRevisarHerramienta, '->', lbValorHerramienta, new Ext.Toolbar.Separator(), ' ', sfBuscarHerramienta],
...
function markToReviewHerramienta(grid, record, rowIndex) {
    if (record.data.recordEditor)
        return;
    else {
        reviewPedidoHerramienta();
    }
}
...
function reviewPedidoHerramienta() {
    function confirmar(btn) {
        if (btn == 'ok') {
            reviewOK();
        }
    ...
    function reviewOK() {
        var winObservacionHerramienta = new Ext.Window({
            ...
            modal: true,
            closeAction: 'close',
            items: [{xtype: 'textarea', anchor: '100%', id: 'observacionherramienta',
            ...
            buttons: [new Ext.Button({
                    ...
                    handler: function() {
                        winObservacionHerramienta.close();
                    }
                })]
        });
        winObservacionHerramienta.show();
        function goToRewiew() {
            if (Ext.getCmp('observacionherramienta').isValid()) {
                ...
                Ext.Ajax.request({
                    ...
                    params: {
                        idpedido: smpedidoHerramienta.getSelected().data.idpedido,
                        status: 2,
                        obs: Ext.getCmp('observacionherramienta').getValue()
                    }
                });
            }
        }
