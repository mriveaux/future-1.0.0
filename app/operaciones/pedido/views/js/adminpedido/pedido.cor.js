...
var btnRevisarCorrectivo = new Ext.Button({
    id: 'btnRevisarCorrectivo',
    text: '<i class="fa fa-search-plus bluedark-button"></i> Revisar',
    tooltip: 'Marcar para revisar',
    handler: function() {
        var record = Ext.getCmp('gppedidoCorrectivo').getSelectionModel().getSelected();
        markToReviewCorrectivo(Ext.getCmp('gppedidoCorrectivo'), record, record);
    }
});
var btnImportar = new Ext.Button({
    id: 'btnImportar',
    text: '<i class="fa fa-upload bluedark-button"></i> Importar',
    tooltip: 'Importar pedidos para el a&ntilde;o',
    handler: importData
});
...
//    autoExpandColumn: 'idproducto',
    ...
    tbar: [btnAdicionarCorrectivo, btnModificarCorrectivo, btnEliminarCorrectivo, btnRevisarCorrectivo, btnImportar, '->', lbValorCorrectivo, new Ext.Toolbar.Separator(), ' ', sfBuscarCorrectivo],
...
function markToReviewCorrectivo(grid, record, rowIndex) {
    if (record.data.recordEditor)
        return;
    else {
        reviewPedidoCorrectivo();
    }
}
...
function reviewPedidoCorrectivo() {
    function confirmar(btn) {
        if (btn == 'ok') {
            reviewOK();
        }
    }
    MensajeInterrogacion('Revisar pedido', String.fromCharCode(191) + 'Est&aacute; seguro que desea enviar a revisi&oacute;n el pedido seleccionado?', confirmar);

    function reviewOK() {
        var winObservacionCorrectivo = new Ext.Window({
            ...
            modal: true,
            closeAction: 'close',
            items: [{xtype: 'textarea', anchor: '100%', id: 'observacioncorrectivo',
            ...
            buttons: [new Ext.Button({
                    ...
                    handler: function() {
                        winObservacionCorrectivo.close();
                    }
                })]
        });
        winObservacionCorrectivo.show();
        function goToRewiew() {
            if (Ext.getCmp('observacioncorrectivo').isValid()) {
                ...
                Ext.Ajax.request({
                    ...
                    params: {
                        idpedido: smpedidoCorrectivo.getSelected().data.idpedido,
                        status: 2,
                        obs: Ext.getCmp('observacioncorrectivo').getValue()
                    }
                });
            }
        }
...
function importData() {
    var winImportCmp = new PrintData(new Object(), {
        reportType: "EXCEL",
        typeAction: 'import'
    });
    winImportCmp.show();
    winImportCmp.doLayout();
}
