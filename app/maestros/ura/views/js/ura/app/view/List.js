/* global Ext */
Ext.define('Ura.view.List', {
    extend: 'Ext.grid.Panel',
    alias: 'widget.uralist', selType: 'rowmodel', columnLines: true,
    initComponent: function() {
        var grid = this;
        this.reUra = Ext.create('Ext.grid.plugin.RowEditing', {
            clicksToEdit: 2,
            listeners: {
                ...
                edit: function(rowEditing, context) {
                    if (context.record.phantom && context.record.data.ura == '') {
                        grid.getStore().remove(context.record);
                    }
                    else {
        ...
        Ext.apply(grid, {
            store: 'Ura', plugins: [this.reUra], disabled: true,
            dockedItems: [{xtype: 'toolbar', id: 'topbar',
                    items: [
                        ...
                        '->', {
                            ...
                            id: 'sfUra',
                            fnOnSearch: function() {
                                var store = Ext.data.StoreManager.lookup('Ura');
                                store.clearFilter(true);
                                grid.down('#pagin-grid').moveFirst();
                                store.load({params: {start: 0, limit: 20, criterio: Ext.getCmp('sfUra').getValue()}});
                            },
                            fnOnClear: function() {
                                var store = Ext.data.StoreManager.lookup('Ura');
                                store.load({params: {start: 0, limit: 20, criterio: Ext.getCmp('sfUra').getValue()}});
            ...
            columns: [
                {xtype: 'rownumberer'},
                {header: 'Ura', dataIndex: 'ura', flex: 1, field: {xtype: 'textfield'}, sortable: true},
                {dataIndex: 'identidad', hideable: false, hidden: true, sortable: false},
                {dataIndex: 'idura', type: 'int', useNull: true, hideable: false, hidden: true, sortable: false},
                {header: futureLang.lbAcciones, align: 'center', xtype: 'actioncolumn', width: 60, sortable: false, menuDisabled: true, items: [{
                            ...
                            handler: function(grid2, rowIndex) {
                                if (grid.getSelectionModel().hasSelection()) {
                                    ...
                                    grid.reUra.startEdit(selection, 0);
                                } else {
            ...
            bbar: {xtype: 'feetpagingtoolbar', id: 'pagin-grid', pageSize: 20, store: 'Ura', displayInfo: true, dock: 'bottom', plugins: new Ext.ux.ProgressBarPager()}
        });
