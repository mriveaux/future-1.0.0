            ...
            items: [{
                    ...
                    items: [{columnWidth: 1,
                            ...
                            html: '<div class="icon-location"><img src="/lib/js/Extjs/2.2/resources/images/icon-license.png" width="100">\n\
                                        ...
                                        <div class="inner">\n\
                                            <button type="submit" class="btn btn-default" onclick="validateRegister(false)">Verificar</button>\n\
                                        </div>\n\
                                        <div class="inner">\n\
                                            <button type="submit" class="btn btn-default btn-save" onclick="validateRegister(true)">Registrar</button>\n\
                                        </div>'
                        }]
                }]
        }, {
            ...
            items: [{
                    ...
                    items: [{columnWidth: 1,
                            ...
                            html: '<div class="instructions"><p>A continuación debe introducir los datos para la conexión a su base de datos. Si no está seguro de esta información contacte con su proveedor de alojamiento web.</p></div></br>\n\
                                        ...
                                        <div class="inner">\n\
                                            <label> Informaci&oacute;n recibida de su proveedor de alojamiento web. </label>\n\
                                        </div></br>\n\
                                        <div class="inner">\n\
                                        ...
                                        <div class="inner">\n\
                                            <button type="submit" class="btn btn-default" onclick="validarCampos(true)">Conectar</button>\n\
                                        </div>\n\
                                        <div class="inner">\n\
                                            <button type="submit" class="btn btn-default btn-save" onclick="validarCampos(false)">Guardar</button>\n\
                                        </div>'
                        }]
                }]
        }, {
            id: 'card-3',
            html: '<div class="end-wizard-title">Finalizaci&oacute;n del asistente de instalaci&oacute;n<hr size="2" color="#D4E1F2"></div>\n\
                    <div class="instructions">\n\
                    ...
                    <div class="inner">\n\
                        <button type="submit" class="btn btn-default" onclick="goToApplication()">Probar sistema</button>\n\
                    </div>'
        }]
});
card.render(document.getElementById('render-wizard'));

function validarCampos(test) {
    var isValid = true;
    if (document.getElementById('dbname').value == '') {
        isValid = false;
        document.getElementById('dbname').setAttribute('style', 'border-color: #FF0000; box-shadow: 0 1px 1px rgba(255, 0, 0, 0.075) inset, 0 0 5px rgba(255, 0, 0, 0.6);');
        document.getElementById('dbname').focus();
    }
    if (document.getElementById('dbuser').value == '') {
        isValid = false;
        document.getElementById('dbuser').setAttribute('style', 'border-color: #FF0000; box-shadow: 0 1px 1px rgba(255, 0, 0, 0.075) inset, 0 0 5px rgba(255, 0, 0, 0.6);');
        document.getElementById('dbuser').focus();
    }
    if (document.getElementById('dbpass').value == '') {
        isValid = false;
        document.getElementById('dbpass').setAttribute('style', 'border-color: #FF0000; box-shadow: 0 1px 1px rgba(255, 0, 0, 0.075) inset, 0 0 5px rgba(255, 0, 0, 0.6);');
        document.getElementById('dbpass').focus();
    }
    if (document.getElementById('dbhost').value == '') {
        isValid = false;
        document.getElementById('dbhost').setAttribute('style', 'border-color: #FF0000; box-shadow: 0 1px 1px rgba(255, 0, 0, 0.075) inset, 0 0 5px rgba(255, 0, 0, 0.6);');
        document.getElementById('dbhost').focus();
    }
    if (document.getElementById('dbport').value == '') {
        isValid = false;
        document.getElementById('dbport').setAttribute('style', 'border-color: #FF0000; box-shadow: 0 1px 1px rgba(255, 0, 0, 0.075) inset, 0 0 5px rgba(255, 0, 0, 0.6);');
        document.getElementById('dbport').focus();
    }
    if (isValid && test) {
        testConnection();
 ...
 * @returns {undefined}
 */
function quitInvalid(idconmponent) {
    if (document.getElementById(idconmponent).value != '') {
        document.getElementById(idconmponent).setAttribute('style', 'border-color:#5599bb;');
    } else
        document.getElementById(idconmponent).setAttribute('style', 'border-color: #FF0000; box-shadow: 0 1px 1px rgba(255, 0, 0, 0.075) inset, 0 0 5px rgba(255, 0, 0, 0.6);');
}
...
function validateRegister(register) {
    ...
    if (document.getElementById('productname').value == '') {
        isValid = false;
        document.getElementById('productname').setAttribute('style', 'border-color: #FF0000; box-shadow: 0 1px 1px rgba(255, 0, 0, 0.075) inset, 0 0 5px rgba(255, 0, 0, 0.6);');
        document.getElementById('productname').focus();
    }
    if (document.getElementById('enterprisename').value == '') {
        isValid = false;
        document.getElementById('enterprisename').setAttribute('style', 'border-color: #FF0000; box-shadow: 0 1px 1px rgba(255, 0, 0, 0.075) inset, 0 0 5px rgba(255, 0, 0, 0.6);');
        document.getElementById('enterprisename').focus();
    }
    if (document.getElementById('serial').value == '') {
        isValid = false;
        document.getElementById('serial').setAttribute('style', 'border-color: #FF0000; box-shadow: 0 1px 1px rgba(255, 0, 0, 0.075) inset, 0 0 5px rgba(255, 0, 0, 0.6);');
    ...
    if (isValid && !register) {
        ...
        if (document.getElementById('validtime').value == '') {
            registerValid = false;
            document.getElementById('validtime').setAttribute('style', 'border-color: #FF0000; box-shadow: 0 1px 1px rgba(255, 0, 0, 0.075) inset, 0 0 5px rgba(255, 0, 0, 0.6);');
            document.getElementById('validtime').focus();
        }
        if (document.getElementById('registerdate').value == '') {
            registerValid = false;
            document.getElementById('registerdate').setAttribute('style', 'border-color: #FF0000; box-shadow: 0 1px 1px rgba(255, 0, 0, 0.075) inset, 0 0 5px rgba(255, 0, 0, 0.6);');
            document.getElementById('registerdate').focus();
        }
        if (document.getElementById('endtime').value == '') {
            registerValid = false;
            document.getElementById('endtime').setAttribute('style', 'border-color: #FF0000; box-shadow: 0 1px 1px rgba(255, 0, 0, 0.075) inset, 0 0 5px rgba(255, 0, 0, 0.6);');
    ...
    Ext.Ajax.request({
        ...
        callback: function(options, success, response) {
            ...
            if (responseData.validSerial) {
                document.getElementById('validtime').value = responseData.validDays;
                document.getElementById('registerdate').value = responseData.registerDate;
                document.getElementById('endtime').value = responseData.endDate;
