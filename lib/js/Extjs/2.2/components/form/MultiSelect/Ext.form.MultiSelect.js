 ...
 *    2016-08-05 Modified by belcross
 * 
 * @constructor
...
Ext.form.MultiSelect = Ext.extend(Ext.form.Field, {
    /**
     ...
     */
    minLengthText: 'Requerido al menos {0} elementos(s)',
    /**
     ...
     */
    maxLengthText: 'Requerido cuando m&aacute;s {0} elementos(s)',
    /**
    ...
    initComponent: function () {
        ...
        return Ext.form.MultiSelect.superclass.initComponent.call(this);
    },
    onRender: function (ct, position) {
        Ext.form.MultiSelect.superclass.onRender.call(this, ct, position);
        ...
        var fs = new Ext.form.FieldSet({
            ...
            title: this.legend,
            autoHeight: true,
            style: "padding:0; padding-bottom:15px; margin:0;",
            tbar: this.tbar
        });
    ...
    onViewClick: function (vw, index, node, e) {
        ...
        this.fireEvent('change', this, this.getValue(), this.getDisplayValue(), index, this.hiddenField.dom.value);
    ...
    /**
     * Returns an array of data display values for the selected items in the list. The values will be separated
     * @param {String/Array} displayField The value or values to show.
     * @return {Array} value An array of string data display values.
     */
    getDisplayValue: function (displayField) {
        var returnArray = [];
        ...
        for (var i = 0; i < selectionsArray.length; i++) {
            returnArray.push(this.store.getAt(selectionsArray[i]).get(((displayField != null) ? displayField : this.displayField)));
        }
        return returnArray.join(this.delimiter);
    },
    /**
     * Returns an array of data values for the selected items in the list. The values will be separated
     * @param {String/Array} valueField The real value.
    ...
    /**
     * Sets a delimited string (using {@link #delimiter}) or array of data dsplay values into the list.
     * @param {String/Array} values The values to set
     */
    setDisplayValue: function (values) {
        var index;
        var selections = [];
        this.view.clearSelections();
        this.hiddenField.dom.value = '';
        ...
        for (var i = 0; i < values.length; i++) {
            index = this.view.store.indexOf(this.view.store.query(this.displayField,
                    new RegExp('^' + values[i] + '$', "i")).itemAt(0));
            selections.push(index);
        }
        this.view.select(selections);
